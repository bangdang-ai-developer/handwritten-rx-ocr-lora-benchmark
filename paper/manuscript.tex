...
\begin{center}
\textit{Corresponding author: Bang Dang (\href{mailto:bang007112345@gmail.com}{bang007112345@gmail.com})}\\[2pt]
\textit{ORCID: \href{https://orcid.org/0009-0005-2952-8807}{0009-0005-2952-8807}}\\[2pt]
\textit{Target venue: PeerJ Computer Science (AI Application track)}
\end{center}

\begin{abstract}
Handwritten prescriptions are a recognized source of medication error, making automatic transcription
...
risk, namely the model inserting memorized drug names into ordinary text it had never been trained on. The
code, frozen evaluation data, and the trained adapter are released to support exact reproduction on free-tier
cloud GPUs.
...
generations of approaches. Early systems relied on binarization and template matching. These gave way to
...
altogether, training an end-to-end image-to-text transformer on synthetic full-page documents. Most
...
rank held at 16, for augmentation on versus off. To avoid choosing the configuration reported as ``the''
fine-tuned model by peeking at the frozen test sets, we select the primary rank on the full, held-out
Kaggle-Rx \texttt{Validation} split (n = 780, disjoint from both \texttt{Training} and \texttt{Testing} ---
not to be confused with the 300-image random subsample of the same split used only for epoch-level early
stopping during training) rather than by test-set CER: r = 32 has the lowest validation CER (0.117),
followed by r = 16 (0.135) and r = 8 (0.156), so r = 32 is the configuration we report as primary and carry
into Section~4.2. The frozen-test-set ablation numbers in Table~\ref{tab:ablation} are reported separately,
as a descriptive comparison, and were not used to make this selection. One caveat we want to state plainly
rather than bury: each configuration in Table~\ref{tab:ablation} was trained with a single random seed, so
part of the spread we observe between ranks could reflect ordinary run-to-run training variance rather than
a pure rank effect. We checked this with a partial replication: two further training runs per rank at seeds
123 and 2024, under a corrected seeding procedure that fixes a bug where the configured seed value did not
previously affect LoRA weight initialization (Table~\ref{tab:seedsweep} in Section~4.2 gives the full mean
$\pm$ SD; Section~6 discusses the implications and important caveats, including that r = 8's third run also
uses this corrected procedure while r = 16 and r = 32's original runs predate it). The corrected-procedure
rerun of r = 8 alone, for instance, produced a noticeably worse test-set CER (0.162) than the original r = 8
run reported in Table~\ref{tab:ablation} (0.113) --- underscoring that single-run rank comparisons should be
read with caution regardless of which split they are computed on. Across all three runs per rank, r = 32
keeps the lowest mean CER on both test sets, but the seed-to-seed spread is large enough that we treat this
as support for r = 32 as the better choice on average rather than as evidence it would win on every
individual run (Section~6). The paired Wilcoxon tests in Section~4.2 establish
that the specific checkpoints we originally compared are statistically distinguishable, which is a narrower
and weaker claim than that repeated training runs at a given rank would reliably reproduce the same ranking.
...
choice, not a clinically validated cutoff, and verify with a post-hoc sensitivity sweep that the
confusable-wrong-drug category remains non-negligible across a range of plausible cutoffs (Section~6). For IAM, which has no closed vocabulary, we instead
...
absolute terms, and the independent validation-set selection in Section~3.4 agreeing on r = 32 --- to justify
using r = 32 as ``the fine-tuned model'' for the rest of this paper. The r = 8 vs.\ r = 16 comparison above is
statistically significant \emph{for the specific pair of single-seed checkpoints we trained}, but should be
read with the caveat already flagged in Section~3.4 and revisited in Section~6: a reseeded rerun of r = 8
alone moved its Kaggle-Rx CER from 0.113 to 0.162 (worse than r = 16's 0.149), so we do not treat ``r = 8
beats r = 16'' as an established rank effect, only as a property of this particular pair of training runs.
Turning
...
\begin{table}[ht]
\centering
\caption{Multi-seed replication: CER, mean $\pm$ SD across three training runs per rank (the original run
in Table~\ref{tab:ablation}, plus two further runs at seeds 123 and 2024 under the corrected seeding
procedure of Section~6; for r = 8, whose original checkpoint was not preserved, all three runs use the
corrected procedure). Elastic augmentation on throughout.}
\label{tab:seedsweep}
\small
\begin{tabular}{lcc}
\toprule
Rank & CER, Kaggle-Rx (mean $\pm$ SD) & CER, IAM (mean $\pm$ SD) \\
\midrule
r = 8  & 0.159 $\pm$ 0.013 & 0.551 $\pm$ 0.072 \\
r = 16 & 0.144 $\pm$ 0.015 & 0.536 $\pm$ 0.048 \\
r = 32 & 0.130 $\pm$ 0.016 & 0.529 $\pm$ 0.029 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Multi-seed replication.} The Kaggle-Rx and IAM single-seed CER values quoted above are also
summarized, with two further training runs per rank, in Table~\ref{tab:seedsweep} (mean $\pm$ SD across
three runs; full detail in Section~6). r = 32 keeps the lowest mean CER on both datasets (Kaggle-Rx: 0.130
$\pm$ 0.016; IAM: 0.529 $\pm$ 0.029), consistent with the independent validation-set selection in
Section~3.4 --- note this is not the same as saying r = 32 has the lowest single-run test CER in
Table~\ref{tab:ablation} (r = 8's 0.113 is nominally lower there than r = 32's 0.114), which is exactly the
kind of single-run noise this replication is meant to average over. The per-rank spread is large relative to
the gaps between ranks: on Kaggle-Rx, r = 16's mean (0.144 $\pm$ 0.015) and r = 32's mean (0.130 $\pm$ 0.016)
sit less than one SD apart, and on one of the three runs per rank (seed 123) r = 16 actually beats r = 32
outright (0.127 versus 0.146); the same reversal appears on IAM at seed 2024 (r = 16: 0.495, r = 32: 0.560).
We therefore read this replication as supporting r = 32 as the better choice \textit{on average}, not as
evidence that r = 32 would beat r = 16 on every possible training run --- the same caution already flagged
for the r = 8 vs.\ r = 16 comparison in Section~3.4 applies here too, just less severely.
...
closer to the unrelated real drug ``Axodin'' than to the correct answer (Table~\ref{tab:qualitative} shows
...
\begin{table}[ht]
\centering
\caption{Representative fine-tuned-model (r = 32) predictions on Kaggle-Rx, one example per error category
from Table~\ref{tab:errortax}. Reported as reference/prediction text rather than image crops: the underlying
handwriting images are third-party content from the Kaggle-Rx dataset \citep{Mia2024KaggleRx}, released
under the Open Database License with contents copyright reserved to the original authors, and we have not
separately obtained reproduction permission for specific images; the drug names and model outputs
themselves are factual strings, not copyrightable expression.}
\label{tab:qualitative}
\begin{tabular}{llll}
\toprule
Category & Reference & Fine-tuned prediction & Nearest vocabulary match \\
\midrule
Correct & Baclofen & Baclofen & Baclofen \\
Correct & Denixil & Denixil & Denixil \\
Confusable wrong drug & Amodis & Amodin & Axodin \\
Confusable wrong drug & Etizin & Eitrin & Zithrin \\
Hallucination, far off & Alatrol & KKKKK & Beklo \\
Minor OCR noise, still correct drug & Aceta & Acata & Aceta \\
\bottomrule
\end{tabular}
\end{table}
...
is a common default in the fine-tuning literature, and it was our own initial plan. Our ablation, and
independently the validation-set selection in Section~3.4, both show that r = 32 beats r = 16 on the
in-domain metric, with no compensating advantage on generalization; we flag this mainly as a methodological
caution: without checking, we would have reported a materially weaker headline result (CER 0.149 rather than
0.114) under the mistaken impression that r = 16 was close to optimal. The single-run ablation additionally
suggested r = 8 also beat r = 16, but that specific comparison is the one a reseeded rerun (Section~3.4,
Section~6) failed to reproduce, so we no longer treat ``r = 8 beats r = 16'' as established -- only
``r = 32 beats r = 16,'' which two independent selection procedures agree on, and which a subsequent
three-run-per-rank replication (Section~6) also supports on average (mean Kaggle-Rx CER 0.130 for r = 32
versus 0.144 for r = 16), albeit with individual-run variance large enough that r = 16 wins outright on one
of the three runs.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig10_threshold_sensitivity.png}
\caption{Sensitivity of the confusable-drug distance threshold: distribution of normalized edit distances
for wrong, non-degenerate, top-1-incorrect Kaggle-Rx predictions (n = 55), with the 0.34 cutoff used in the
paper and the data-driven largest-gap candidate ($\approx$0.21) marked.}
...
vocabularies.
\item \textbf{A partial, three-seed replication, not the five-seed design we judge would be needed.}
Each of the four original ablation configurations (Sections~3.4, 4.2) was trained exactly once. The paired
Wilcoxon tests tell us the specific trained checkpoints differ significantly on the frozen test sets; they do
not tell us that repeated training runs at a given rank would reliably reproduce the same ranking, and part
of the observed rank effect could be run-to-run training variance. We partially addressed this for the three
ranks (8, 16, 32; the augmentation-off r = 16 configuration remains single-seed): for each rank we now have
three training runs --- the original run in Table~\ref{tab:ablation}, plus two further runs at seeds 123 and
2024 under a corrected seeding procedure that fixes a bug where the configured seed value did not previously
affect LoRA weight initialization. (For r = 8, whose original checkpoint was not preserved on disk, all
three runs use the corrected procedure, i.e.\ three genuinely controlled seeds; for r = 16 and r = 32, the
first run predates the fix, so it reflects whatever uncontrolled randomness affected LoRA initialization at
the time rather than a deliberately chosen seed, and is included as the best available third data point, not
as a methodologically clean seed.) Table~\ref{tab:seedsweep} (Section~4.2) reports the resulting mean $\pm$
SD: r = 32 keeps the lowest mean CER on both Kaggle-Rx (0.130 $\pm$ 0.016) and IAM (0.529 $\pm$ 0.029),
consistent with the validation-set selection, but the spread across runs is large enough that r = 16 wins
outright on one of the three runs on both datasets (Kaggle-Rx CER 0.127 versus r = 32's 0.146 at seed 123;
IAM 0.495 versus 0.560 at seed 2024). We therefore continue to treat ``r = 32 beats r = 16'' as favored on
average rather than decisively established per-run, and, as already noted in Sections~4.2 and~5, no longer
treat ``r = 8 beats r = 16'' as established at all: one of r = 8's own three runs (seed 2024, CER 0.146)
already beats the original single-run r = 16 result (0.149) reported in Table~\ref{tab:ablation}. A full
replication with five seeds per rank and the same Wilcoxon/Holm--Bonferroni/rank-biserial pipeline
(Section~3.3) re-run across seeds, which we judge would be needed to formally establish rather than merely
support a rank ordering, remained beyond the free-tier GPU budget of this study.
...
\item \textbf{The 0.34 confusable-drug distance threshold (Section~3.5) is a reasonable choice within a
range that supports our conclusion, not a uniquely optimal or clinically validated cutoff.} We swept
candidate thresholds from 0.20 to 0.50 against the 55 wrong, non-degenerate, top-1-incorrect Kaggle-Rx
predictions the split applies to (Figure~\ref{fig:threshold_sensitivity}). A purely data-driven
largest-gap heuristic, searched over the wider distance range [0.15, 0.55], points to $\approx$0.21 rather
than 0.34, so we do not claim 0.34 sits at a clean natural break in the distance distribution. What the
sweep does show is that the fine-tuned model's confusable-wrong-drug category stays non-negligible across
the whole plausible range: its share of all 780 Kaggle-Rx predictions runs from 1.9\% (at 0.20) to 5.5\% (at
0.50), never disappearing, though the specific point estimate we report (2.6\% at 0.34,
Table~\ref{tab:errortax}) is one value within that range rather than a uniquely determined one; we did not
re-run the equivalent sweep for the zero-shot model, so this check supports the category's existence being
robust to the exact cutoff, not that the specific zero-shot-versus-fine-tuned shift in
Table~\ref{tab:errortax} is itself threshold-invariant. We therefore report 0.34 as a transparent, reasonable
design choice supported by a robustness check, not as a precisely justified cutoff; a formal LASA-pair-based
definition, following Karet \citep{Karet2023TallMan}, would be a natural refinement.
...
adapter weights are available at
\url{https://github.com/bangdang-ai-developer/handwritten-rx-ocr-lora-benchmark} and are archived at Zenodo
under DOI \url{https://doi.org/10.5281/zenodo.22822499} (release v1.0.1), which corresponds exactly to the
code and results reported here. Both datasets are public: Kaggle-Rx
...
throughout this project's development (15 September--23 September 2026) to assist with running the
benchmark and fine-tuning pipeline, drafting and editing manuscript prose, writing and reviewing the
statistical-analysis code described in Section~3.3 (\texttt{src/analyze\_\allowbreak statistics.py}),
generating the figures in Section~4, and literature search and citation triage. Specifically: Claude Sonnet
5 (model ID claude-sonnet-5) was used for the majority of the project, including the initial benchmark and
fine-tuning pipeline (15--16 September 2026), the LaTeX manuscript draft and majority of code commits
(17--18 September 2026), and the subsequent methodology revisions, multi-seed replication, and final editing
passes reported here (21--23 September 2026); Claude Opus 5 (model ID claude-opus-5) was used briefly on 17
September 2026 for literature research on academic submission standards and authentic-writing/AI-disclosure
norms, and for an initial draft of the statistical-analysis script, before the author reverted to Sonnet 5
for the remainder of the project; Claude Haiku 4.5 (model ID claude-haiku-4-5-20251001) was used for
portions of manuscript review and venue-verification research on 17--18 September 2026.
The author (B.\,Dang) has reviewed Anthropic's terms of service and usage policies for Claude and confirms
that these tools are suitable for use in preparing this manuscript for publication. The author directed all
research decisions, designed and ran every experiment, independently verified every statistic, figure, and
citation reported in this manuscript (including the accuracy of all references), and takes full
responsibility for the integrity of the whole content. No generative AI tool is listed as an author, and no
AI tool was used to create, alter, or fabricate any research image or experimental data.

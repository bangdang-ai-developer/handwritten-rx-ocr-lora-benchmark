...
rather than bury: each configuration was trained with a single random seed, so part of the spread we observe
between ranks could reflect ordinary run-to-run training variance rather than a pure rank effect
(Section~6). Consistent with that concern, a rerun of the r = 8 configuration under a corrected seeding
procedure (Section~6) produced a noticeably worse test-set CER (0.162) than the original r = 8 run reported
in Table~\ref{tab:ablation} (0.113), underscoring that single-run rank comparisons should be read with
caution regardless of which split they are computed on. The paired Wilcoxon tests in Section~4.2 establish
that the specific checkpoints we compared are statistically distinguishable, which is a narrower and weaker
claim than that repeated training runs at a given rank would reliably reproduce the same ranking.
...
``r = 32 beats r = 16,'' which two independent selection procedures agree on.
...
ordering reflects a true rank effect rather than seed noise; a full replication of this scope was beyond the
free-tier GPU budget of the study, though a partial check is informative: rerunning r = 8 alone under a
corrected seeding procedure (Section~3.4) moved its test-set CER from 0.113 to 0.162, a swing large enough on
its own to overturn the r = 8-over-16 part of the ordering, which is direct evidence that seed noise, not
only a true rank effect, contributes materially to the ablation spread we report.

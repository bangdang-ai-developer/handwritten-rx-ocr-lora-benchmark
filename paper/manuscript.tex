...
\textit{Target venue: PeerJ Computer Science (primary); Journal of Imaging and Health Information Science and Systems (alternates)}
\end{center}

\vspace{6pt}
\begin{quote}
\small\textit{Note to author, remove before submission: confirm the name/affiliation string above and add
an ORCID if available. Two references in the bibliography (RxScribe Bench author list; Chen et al.,
IEEE/ACM TASLP full citation) are marked as unconfirmed and must be verified against the primary source
...
model, and current-generation general VLMs --- on a \textit{public} prescription dataset. The nearest
comparable work, RxScribe Bench \citep{RxScribeBench2026}, evaluates frontier VLMs on handwritten
prescriptions but does so on a newly collected \textit{private} dataset and does not test fine-tuning. As
far as we can tell, ours is the first study to combine a fully public dataset, this specific model lineup,
and a lightweight fine-tuning step with an explicit check of how much general handwriting ability it costs.
...
behave differently from full fine-tuning even at comparable in-domain accuracy; and Chen et
al.\ \citep{Chen2024BayesianPEFT} state plainly that catastrophic forgetting ``remains an issue with PEFT''
and propose a Bayesian regularizer to address it, while Kalajdzievski
...
partial, degradation on out-of-domain data, which sits closer to Chen et al.'s framing than to an unqualified
reading of ``LoRA forgets less.''
...
forgetting analysis. RxScribe Bench \citep{RxScribeBench2026}, published within weeks of the present work,
benchmarks frontier VLMs on a newly collected private set of Indian outpatient prescriptions along four
clinical-risk axes, but neither fine-tunes a model nor uses public data. A hybrid
...
(\texttt{mamun1113/doctors-handwritten-prescription-bd-dataset} on Kaggle) contains cropped, single-word
...
\texttt{nibinv23/iam-handwriting-word-database}. IAM serves two purposes here: a domain-shift baseline
...
single Kaggle notebook session with two T4 GPUs under automatic \texttt{DataParallel}.
...
purpose-built GOT-OCR2.0 (roughly 580M parameters). Taken at face value this is a striking result: a model
...
and GOT-OCR2.0 on IAM is \textit{not} significant, $p = 0.330$, despite the large gap in means), and a
caution for anyone scoring general VLMs as OCR systems using mean error alone.
...
configuration was trained once, to early stopping, on identical data and seed.}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
Configuration & CER, Kaggle-Rx & CER, IAM & $p$ vs.\ r=16 elastic (Rx / IAM) \\
\midrule
r = 8, elastic & 0.113 [0.098, 0.128] & 0.530 [0.475, 0.580] & $1.0\times10^{-7}$ / $0.830$ \\
r = 16, elastic & 0.149 [0.132, 0.166] & 0.524 [0.472, 0.579] & --- \\
\textbf{r = 32, elastic (selected)} & \textbf{0.114} [0.100, 0.130] & \textbf{0.501} [0.453, 0.552] & $2.4\times10^{-8}$ / $0.079$ \\
r = 16, no elastic & 0.150 [0.134, 0.167] & 0.562 [0.505, 0.627] & $0.647$ / $0.101$ \\
\bottomrule
\end{tabular}
\end{table}

The a priori ``middle'' choice, r = 16, turns out to be beaten on Kaggle-Rx by both r = 8 and r = 32 (both
$p < 10^{-6}$), and r = 32 additionally posts the best point estimate on IAM, though that specific
r = 32-versus-r = 16 comparison on IAM falls just short of the conventional 0.05 threshold ($p = 0.079$).
Since r = 32 is at least as good as every other configuration we tried on both axes, with no trade-off we
could find, we use it as ``the fine-tuned model'' for the rest of this paper. Turning elastic distortion
off, with rank held at 16, leaves in-domain CER essentially where it was (0.149 versus 0.150,
$p = 0.647$) but produces a worse point estimate on IAM (0.562 versus 0.524), again short of significance
($p = 0.101$); we kept elastic distortion in the final pipeline since there is no sign it hurts and a
non-significant hint that it helps retention.

Training at r = 32 reaches its best validation CER (0.102 on the 300-image tracking subsample) at epoch~2 of
15, and early stopping triggers at epoch~7 after five epochs without improvement (Figure~2). Validation loss
keeps falling past epoch~2 even as validation CER plateaus and then worsens, which is consistent with the
model starting to overfit the small training set on the loss objective well before that shows up in the CER
metric.
...
top-1 accuracy from 79.5\% to 92.4\%. This 558M-parameter fine-tuned model beats every zero-shot system we
tested on this domain, including systems with far more parameters or explicit OCR specialization: against
Qwen2.5-VL-3B-Instruct (3B parameters), $p = 4.4\times10^{-59}$; against GOT-OCR2.0 (roughly 580M
parameters, built specifically for OCR), $p = 8.0\times10^{-93}$ (paired Wilcoxon signed-rank, n = 780 in
both cases). As far as we know, this is the first demonstration that a small, resource-efficient specialist
model, adapted with under 2\% of its parameters on a single free-tier GPU, can decisively outperform much
larger general-purpose VLMs on a target handwriting domain.
...
\textit{increase} in error, and a statistically significant one ($p = 3.2\times10^{-4}$, paired Wilcoxon,
n = 400). The median CER on IAM rises too, from 0.0 zero-shot to 0.400 fine-tuned, which tells us this is a
broad, systematic shift rather than a handful of outlier failures. Figure~5 plots the resulting reversal in
domain-shift gap (CER on IAM minus CER on Kaggle-Rx) across every model: zero-shot OCR-specialist models
(TrOCR, GOT-OCR2.0) score slightly \textit{better} on IAM than on Kaggle-Rx, a negative gap consistent with
IAM being closer to their original pretraining distribution than prescription handwriting is, while the
fine-tuned model shows a substantial positive gap --- a direct, quantitative signature of the specialization
it has undergone. One thing worth clarifying so Table~2 is not misread against this gap figure: the gap
being similar in size for r = 16 (+0.376) and r = 32 (+0.387) does not contradict r = 32 being better on
\textit{both} datasets in absolute terms. r = 32 improves Kaggle-Rx performance by more than it improves
IAM performance relative to r = 16, and that is enough to widen the gap slightly even as both individual
numbers get better.
...
dimensions'' \citep{Shuttleworth2025Illusion} and Chen et al.'s explicit statement that forgetting
``remains an issue'' for PEFT generally \citep{Chen2024BayesianPEFT}.
...
closer to the unrelated real drug ``Axodin'' than to the correct answer --- looks plausible on its face and,
...
severe error zero-shot to 26.8\% / 32.8\% / 27.5\% fine-tuned --- mostly a shift from exactly correct to
moderately wrong, rather than toward outright failure, which adds some nuance to the mean/median framing of
Section~4.4: fine-tuning on a narrow domain degrades a broad swath of previously perfect out-of-domain
predictions to ``partially wrong'' more than it manufactures new catastrophic failures.
...
never ran a matched full fine-tuning comparison. It fits more comfortably with the more qualified accounts:
Shuttleworth et al.'s ``intruder dimensions'' \citep{Shuttleworth2025Illusion} and Chen et al.'s plain
statement that forgetting remains a live problem for PEFT \citep{Chen2024BayesianPEFT}. Practically, this
...
\textbf{Position relative to the closest related work.} RxScribe Bench \citep{RxScribeBench2026}, published
within weeks of this study, tackles a closely related question --- benchmarking frontier VLMs on
handwritten prescriptions along clinical-risk axes --- but on a newly collected, private dataset of Indian
outpatient prescriptions, without a fine-tuning or forgetting analysis. We see our work as complementary: a
fully public and independently reproducible dataset and pipeline, a directly comparable model set that
includes both a purpose-built OCR-2.0 system and general VLMs, and an explicit test of whether lightweight
fine-tuning changes the qualitative safety profile of the errors that remain, not only their aggregate rate.
...
at \texttt{kaggle.com/datasets/mamun1113/doctors-handwritten-prescription-bd-dataset}, and IAM through its
standard registration process at \texttt{fki.tic.heia-fr.ch/databases/iam-handwriting-database}; readers

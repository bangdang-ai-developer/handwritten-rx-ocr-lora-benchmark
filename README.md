...
**LoRA fine-tune TrOCR-large-handwritten (Phase 7-8, cấu hình chính thức: r=32 sau ablation) ĐÃ XONG** —
kết quả thật, 3.120 ảnh train, so sánh paired với zero-shot (n khớp 100%, Wilcoxon signed-rank):

| Dataset | CER zero-shot → fine-tuned | Exact-match | Wilcoxon p-value | Ý nghĩa |
|---|---|---|---|---|
| **Kaggle-Rx test (in-domain, 780 ảnh)** | 0,580 → **0,114** (−80,3%) | 8,1% → **68,1%** | p=6,97×10⁻¹¹¹ | Cải thiện rất lớn, cực kỳ có ý nghĩa |
| **IAM (out-of-domain, 400 ảnh)** | 0,441 → **0,501** (+13,7%, TỆ HƠN) | 57,3% → 26,8% | p=3,18×10⁻⁴ | Catastrophic forgetting có ý nghĩa thống kê (nhẹ hơn r=16) |

Top-1 accuracy (ánh xạ về 78 tên thuốc thật) sau fine-tune: **92,4%**. Chi tiết đầy đủ (cấu hình LoRA, lịch sử train, diễn giải song song 2 phát hiện) ở [results/phase7_summary.md](results/phase7_summary.md).
...
tiêu (CER 0,114 vs 0,434 vs 0,479, Wilcoxon p<10⁻⁵⁸ cho cả hai so sánh). Chi tiết đầy đủ (bảng 8 model × 2
...
tiêu: lỗi "đoán bừa" (hallucination) giảm ~4,1 lần (18,6%→4,5%), nhưng lỗi "giống một tên thuốc thật khác"
(nguy hiểm hơn về an toàn lâm sàng vì khó bị phát hiện thủ công) tăng ~1,7 lần (1,5%→2,6%). Trên IAM,
catastrophic forgetting chủ yếu là dịch chuyển từ "đúng tuyệt đối" sang "lỗi mức trung bình", không phải sụp
đổ hoàn toàn; kiểm tra riêng cho thấy **không có bằng chứng hệ thống về "rò rỉ" tên thuốc** khi model đọc
chữ viết tay tổng quát (0,5% trên IAM). Chi tiết + ví dụ cụ thể ở [results/phase10_summary.md](results/phase10_summary.md), script tái lập ở [src/analyze_errors.py](src/analyze_errors.py).

**Ablation (Phase 7b) ĐÃ XONG, kết luận đã áp dụng** — chạy full training cho r=8/16/32 (elastic=True) +
r=16 không-elastic. **Phát hiện quan trọng: r=16 (cấu hình "main" ban đầu) KHÔNG phải rank tốt nhất** — cả
r=8 và r=32 đều vượt r=16 có ý nghĩa thống kê mạnh trên kaggle_rx (CER 0,113/0,114 vs 0,149, p<10⁻⁶), và
**r=32 tốt nhất trên cả 2 tiêu chí** (kaggle_rx VÀ giữ tổng quát hoá IAM tốt nhất, không đánh đổi) → **đã
chọn r=32 làm kết quả chính thức của toàn bài**, Phase 7/9/10 đã được viết lại theo r=32. Augmentation
elastic: không ảnh hưởng in-domain, có xu hướng (chưa đạt ý nghĩa 0,05) giảm nhẹ forgetting → giữ
elastic=True. Chi tiết đầy đủ ở [results/phase7b_ablation_summary.md](results/phase7b_ablation_summary.md).

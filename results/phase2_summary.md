...
4. **Donut-base (chỉ pretrain SynthDoG, chưa fine-tune) THẤT BẠI TRÊN CẢ 2 CÁCH THỬ — kết luận đã xác nhận, không phải lỗi code:**
   - **Raw** (crop đưa thẳng vào): degenerate rate ~100% cả 2 dataset (output rỗng hoàn toàn).
   - **Padded** (dán crop vào giữa 1 "trang trắng" mô phỏng tỷ lệ khung hình SynthDoG được train): degenerate rate giảm còn 77.8% (kaggle_rx) / 96.5% (IAM) — tức đôi khi model chịu sinh ra chữ — nhưng khi sinh, CER trung bình lên tới **4.20** (kaggle_rx) và **1.06** (IAM), tức output dài và sai lệch nặng (hallucination), không phải text đúng.
   - **Kết luận:** đã kiểm chứng bằng 2 cách khác nhau (không phải do khung hình sai lệch) — `donut-base` thuần pretrain SynthDoG, KHÔNG fine-tune, thực sự không tạo ra output OCR hữu dụng ở cấp độ từ đơn lẻ, dù không "im lặng" (raw) hay "nói bậy" (padded). Đây là **phát hiện khoa học hợp lệ, đáng đưa vào bài báo** (minh chứng rằng không phải mọi "OCR-free VLM" đều dùng trực tiếp được zero-shot mà không fine-tune/task-adaptation trước — một luận điểm ủng hộ chính hướng đóng góp fine-tuning của bài báo này), không phải lỗi kỹ thuật cần sửa thêm. Không đầu tư thêm thời gian debug Donut nữa — chuyển sang Phase 3 (GOT-OCR2.0).
